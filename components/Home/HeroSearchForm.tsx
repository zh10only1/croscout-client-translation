  ...
  return (
    ...
    <form onSubmit={handleSearch}>
      ...
      <div className="flex select-none  justify-center absolute left-0 right-0 md:-top-10 -top-14">
        <div className="flex py-2 px-2 cursor-pointer rounded-full active:scale-[.99] duration-200 text-white bg-primary text-sm md:text-lg ">
          {/* Add Location */}
          <AddSearchValueBtn
            id="add-location"
      ...
      <div className="">
        ...
        >
          ...
          <span>
            {location ? t(location) : t("HOME_SEARCH_LOCATION_SELECT")}
          </span>
          {/* Down Arrow Button */}
          <FaChevronDown
            className={`absolute ${heroStyles.downArrow} right-1 lg:right-4 text-xl`}
          />
        </div>
                ...
                {formattedStartDate !== formattedEndDate ? (
                  <>
                    <div className="text-sm lg:text-base lg:leading-5">
                      {t(format(new Date(searchCalDate[0].startDate), "MMM"))}{" "}
                      {format(new Date(searchCalDate[0].startDate), "dd, yyyy")}
                    </div>
                    <div className="text-sm lg:text-base lg:leading-5">
            ...
            <div className="flex items-center gap-2 text-white px-2 lg:px-5  relative">
              ...
              <div>
                {formattedStartDate !== formattedEndDate ? (
                  <>
                    <div className="text-sm lg:text-base lg:leading-5">
                      {t(format(new Date(searchCalDate[0].endDate), "MMM"))}{" "}
                      {format(new Date(searchCalDate[0].endDate), "dd, yyyy")}
                    </div>
                    <div className="text-sm lg:text-base lg:leading-5">
