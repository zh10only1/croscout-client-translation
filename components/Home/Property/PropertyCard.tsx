...
export default function PropertyCard({ property, lng }: Property & any) {
  ...
  function findNextFreeDays(bookingDates: Event[]): string {
    ...
    for (const booking of bookingDates) {
      ...
      if (currentDate < bookingStartDate) {
        const daysDifference = Math.ceil(
          (bookingStartDate.getTime() - currentDate.getTime()) /
            (1000 * 60 * 60 * 24)
        );
        ...
        if (freeDaysCount + daysDifference >= 3) {
          endDate = new Date(
            currentDate.getTime() +
              (3 - freeDaysCount - 1) * 24 * 60 * 60 * 1000
          );
          break;
        }
    ...
    endDate = new Date(
      currentDate.getTime() + (3 - freeDaysCount) * 24 * 60 * 60 * 1000
    );
    ...
    const start_month = t(`${formattedStartDate.split(" ")[0]}`);
    const start_day = formattedStartDate.split(" ")[1];

    const end_month = t(`${formattedEndDate.split(" ")[0]}`);
    const end_day = formattedEndDate.split(" ")[1];
