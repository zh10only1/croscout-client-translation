  ...
  return (
    <div className="h-full">
      <Carousel slide={false}>
        {propertyImages.map((img: any, i: number) => (
          <Link
            className="h-full w-full"
            href={`/${lng}/property-details/${propertyId}`}
            key={i}
          >
            <Image
              key={i}
              ...
              // layout="fill"
              className={
                "cursor-pointer h-full hover:scale-105 duration-200 w-full carouselImage object-cover"
              }
            />{" "}
          </Link>
        ))}
