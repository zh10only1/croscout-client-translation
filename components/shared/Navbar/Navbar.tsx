...
const Navbar = ({ lng }: { lng: string }) => {
  ...
  const { setLoginModal, setSignupModal, sidebarToggle, setSidebarToggle } =
    useModalContext();
  ...
  return (
    <>
      {user && !isVerifed && (
        <div
          hidden={isResetPassword || isVerifyEmail}
          className="bg-red-500 text-white"
        >
          <p className="text-center py-3 text-white">
            <Link href={profileRoute} className="underline">
      ...
      <nav
        hidden={isResetPassword || isVerifyEmail}
        id="topbar"
        className={`py-5   z-40 sticky top-0 ${
          isDashboard ? "bg-[#182237]" : "bg-primary"
        }`}
      >
        {/* Wrapper */}
        <div
          className={` flex-between items-center relative ${
            isDashboard ? "w-full px-6" : "wrapper"
          }`}
        >
          ...
          {/* User Menu Dropdown */}
          <ul
            className={`${navbarStyles.navUserMenu} ${
              navUserToggle ? "scale-y-100" : "scale-y-0"
            }`}
          >
            {user ? (
              <>
                <button className={navbarStyles.dashboardBtn}>
                  {user?.name}
                </button>
